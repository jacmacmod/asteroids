...
app.get('/api/asteroids/namesAndSize', async (req, res) => {
  try {
    const asteroidNames = await db
      .select('name', 'diameter_in_meters_min')
      .table('asteroids');
...
app.get('/api/closest/:name', async (req, res) => {
  ...
  try {
    ...
    const compareable = await db.raw(
      `select name, diameter_in_meters from items order by abs(diameter_in_meters - ${diameter}) limit 1;`
    );
